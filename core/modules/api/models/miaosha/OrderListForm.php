...
class OrderListForm extends Model
{
    ...
    public function search()
    {
        ...
        foreach ($list as $order) {
            ...
            $pay_type_list = OrderData::getPayType($this->store_id, $is_payment, ['balance']);
            $new_list[] = (object)[
                'order_id' => $order->id,
    ...
    private function getRefundList()
    {
        ...
        $count = $query->count();
        $pagination = new Pagination(['totalCount' => $count, 'page' => $this->page - 1, 'pageSize' => $this->limit]);
        $list = $query->select('o.id AS order_id,o.order_no,or.id AS order_refund_id,o.goods_id,or.addtime,o.num,o.total_price,o.attr,or.refund_price,or.type,or.status')->limit($pagination->limit)->offset($pagination->offset)->orderBy('or.addtime DESC')->asArray()->all();
        $new_list = [];
