                    ...
                    <tr>
                        ...
                        <td class="order-tab-4">
                            ...
                            <?php if ($order_item['pay_type'] == 2): ?>
                                <div>
                                    支付方式：
                                    <span class="badge badge-warning">货到付款</span>
                                </div>
                            <?php else: ?>
                                <div>
                                    支付方式：
                                ...
                                <?php if ($order_item['is_delete'] == 0): ?>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-primary apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/miaosha/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 1]) ?>">同意请求</a>
                                    </div>
                                    <div class="mb-2">
                                        <a class="btn btn-sm btn-danger apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/miaosha/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 0]) ?>">拒绝请求</a>
                                    </div>
                                <?php endif; ?>
                            <?php else: ?>
...
<script>
    ...
    $(document).on('click', '.add-price', function () {
        ...
        error.hide();
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/miaosha/order/add-price'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                order_id: order_id,
