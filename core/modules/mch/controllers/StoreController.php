...
class StoreController extends Controller
{
    public function actionIndex()
    {
        if (\Yii::$app->request->isAjax) {
            ...
            return $this->renderJson($store_data);
        } else {
    ...
    public function actionPostageRulesEdit($id = null)
    {
        ...
        if (\Yii::$app->request->isPost) {
            ...
            $province = District::find()->andWhere(['level' => 'province'])->asArray()->all();
            foreach ($province AS $key => $value){
                $province[$key]['city'] = District::find()->andWhere(['level'=>'city','parent_id'=>$value['id']])->asArray()->all();
            }
...
//                Option::set('home_page_data',json_encode($update_list,JSON_UNESCAPED_UNICODE),$this->store->id,'app');
    ...
    public function actionShop()
    {
        ...
        // foreach($arr['list'] as $k => $v){
        //     $orderDetail = Order::find()->where(['store_id' => $this->store->id,'shop_id'=> $v['id'],'is_pay'=> 1])->all();
        //     $pay_price = 0;
    ...
    public function actionWechatSetting()
    {
        if (\Yii::$app->request->isPost) {
            ...
            return $this->render('wechat-setting', [
