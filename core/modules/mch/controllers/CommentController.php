...
class CommentController extends Controller
{
    public function actionIndex()
    {
        ...
        $list = $query
            ...
            ->select('oc.user_id as uid,oc.is_virtual,oc.virtual_user,oc.id,u.nickname,u.avatar_url,oc.score,oc.content,oc.pic_list,g.name goods_name,oc.is_hide,oc.reply_content')
    ...
    { 
        ...
        if (\Yii::$app->request->isPost) {
            ...
            $form->pic_list= json_encode($pic_list, JSON_UNESCAPED_UNICODE);
            ...
            $list = Goods::find()->where(['store_id' => $this->store->id])->all();
            $select = array();
            foreach($list as $k=>$v){
                $select[$k]->name = $v['name'];
                $select[$k]->value = $v['id'];
             }
