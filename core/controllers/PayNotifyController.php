...
//            return;
        ...
        $order->pay_type = 1;
        if ($order->save()) {
            $this->buyData($res['out_trade_no'],$order->store_id);
     ...
     * 购买成功首页提示 
     */
    private function buyData($order_no,$store_id){
        $order = Order::find()->select(['u.nickname','g.name','u.avatar_url'])->alias('c')
    ...
    private function MsOrderNotify($res){
        ...
        $order->pay_type = 1;
        if ($order->save()) {
            $wechat_tpl_meg_sender = new MsWechatTplMsgSender($order->store_id, $order->id, $wechat);
            $wechat_tpl_meg_sender->payMsg();
